...
declare module '@jfrog/tasks-utils' {
    ...
    export async function executeCliTaskAsync(
        runTaskFunc: (cliPath: string) => void,
        cliVersion?: string,
        cliDownloadUrl?: string,
        cliAuthHandlers?: ifm.IRequestHandler[],
    ...
    export async function executeCliCommandAsync(cliCommand: string, runningDir: string, options?: {}): string;
    ...
    export async function createBuildToolConfigFileAsync(
        cliPath: string,
        cmd: string,
        requiredWorkDir: string,
        configCommand: string,
    ...
    export async function configureDefaultXrayServerAsync(usageType: string, cliPath: string, workDir: string): string;
    export function configureDefaultArtifactoryServer(usageType: string, cliPath: string, workDir: string): string;
    export async function configureDefaultArtifactoryServerAsync(usageType: string, cliPath: string, workDir: string): string;
    export function configureDefaultDistributionServer(usageType: string, cliPath: string, workDir: string): string;
    export async function configureDefaultDistributionServerAsync(usageType: string, cliPath: string, workDir: string): string;
    export function taskDefaultCleanup(cliPath: string, workDir: string, serverIdsArray: string[]): string;
    export async function taskDefaultCleanupAsync(cliPath: string, workDir: string, serverIdsArray: string[]): string;
    ...
    export async function removeExtractorsDownloadVariablesAsync(cliPath: string, workDir: string);
    export function configureArtifactoryCliServer(artifactoryService: string, serverId: string, cliPath: string, buildDir: string);
    export async function configureArtifactoryCliServerAsync(artifactoryService: string, serverId: string, cliPath: string, buildDir: string);
